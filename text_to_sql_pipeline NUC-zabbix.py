...
class Pipeline:
    ...
    def __init__(self):
        self.name = "Database RAG Pipeline NUC-zabbix"
        self.engine = None
        self.nlsql_response = ""
        ...
        self.valves = self.Valves(
            **{
                ...
                "DB_HOST": os.getenv("DB_HOST", "http://192.168.1.7"),                     # Database hostname
                "DB_PORT": os.getenv("DB_PORT", 5432),                                        # Database port 
                "DB_USER": os.getenv("DB_USER", "ashton"),                                  # User to connect to the database with
                "DB_PASSWORD": os.getenv("DB_PASSWORD", "woody86"),                          # Password to connect to the database with
                "DB_DATABASE": os.getenv("DB_DATABASE", "zabbix"),                          # Database to select on the DB instance
                "DB_TABLE": os.getenv("DB_TABLE", "events"),                            # Table(s) to run queries against 
                "OLLAMA_HOST": os.getenv("OLLAMA_HOST", "http://192.168.1.7:11434"), # Make sure to update with the URL of your Ollama host, such as http://localhost:11434 or remote server address
